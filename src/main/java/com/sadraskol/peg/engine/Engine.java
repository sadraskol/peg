        ...
        case Declaration.Record record -> {
          ...
          propositions.add(
              new Proposition.Binary(
                  Operator.Equal,
                  new Value.Set(new Set.Named(record.name())),
            ...
            propositions.add(
                new Proposition.Binary(
                    ...
                    new Value.Set(
                        new Set.Product(
                            new Value.Set(new Set.Named(record.name())),
                            new Value.Set(new Set.Named(relation.type()))))));
          }
          types.add(type);
        }
          ...
          case Value.Curried curried -> {
            ...
            if (right instanceof Value.Curried) {
              ...
              if (!curried.set().set().equals(rightNamed)) {
                throw new IllegalStateException(
                    "Expected both curried values to be the same set, got: "
                        + rightNamed
                        + " and: "
                        + curried.set().set());
              }
              ...
              return new Proposition.Exists(
                  List.of(commonVariable),
                  new Value.Set(new Set.Named(setType.type())),
                  new Proposition.And(
      ...
      case Expression.NotEqual notEqual -> {
        ...
        switch (left) {
          case Value.Set _ -> {
            ...
            return new Proposition.Binary(Operator.Different, left, right);
          }
          case Value.Variable _, Value.Member _ -> {
            return new Proposition.Binary(
                Operator.Different, left, evaluateValue(notEqual.right()));
          }
          case Value.Curried curried -> {
            ...
            if (right instanceof Value.Curried) {
              ...
              return new Proposition.Forall(
                  ...
                  new Proposition.Or(
                      new Proposition.And(
                          new Proposition.Binary(Operator.In, leftTuple, tupleSet),
                          new Proposition.Binary(Operator.NotIn, rightTuple, tupleSet)),
                      new Proposition.And(
                          new Proposition.Binary(Operator.In, rightTuple, tupleSet),
                          new Proposition.Binary(Operator.NotIn, leftTuple, tupleSet))));
            } else {
              var relation = new Value.Tuple(List.of(curried.value(), right));
  ...
  private Value evaluateValue(Expression expr) {
    switch (expr) {
      ...
      case Expression.Member member -> {
        ...
        var maybeRelation =
            types.stream().flatMap(type -> type.findRelation(relation.name()).stream()).findFirst();
        var maybeField =
            types.stream().flatMap(type -> type.findField(relation.name()).stream()).findFirst();
        if (maybeRelation.isPresent()) {
          return new Value.Curried(
              new Value.Set(new Set.Named(maybeRelation.get())), evaluateValue(member.callee()));
